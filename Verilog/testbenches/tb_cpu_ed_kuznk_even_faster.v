...
`define tics_count 10000
`define DATA_ROM_SIZE 64
...
initial begin
    $dumpfile("encrypt_kuznyechik_block_even_faster.vcd");
    $dumpvars();
end
integer i;

initial begin
    $readmemh("programs/encrypt_kuznyechik_block_even_faster.hex", program_data);
    $readmemh("precompiled_ci.txt", CI_ROM);
    clk = 0;
    rst = 1;
    ...
    for (i = 0; i < `tics_count; i = i + 1) begin
        ...
        #15;
        $display("tick %d: %d : %h,      %d --- %h", i, progmem_addr[2+$clog2(`PROGSIZE):3], curr_instruction, mem_data_adddr[6:1], data_from_mem);
        clk = 1; $display("UP _/*"); #15; 
        // #10;
        data_from_mem = CI_ROM[mem_data_adddr[6:1]]; #5;
